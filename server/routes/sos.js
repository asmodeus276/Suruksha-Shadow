...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    initiateWorkflow({
      ...
      evidenceCount:         evidenceCount != null ? Number(evidenceCount) : 0,
      blockchainProofHash:   blockchainProofHash || null,
      shareToken:            event.share_token,
    }).catch((err) =>
      console.error("[SOS] Police workflow error (non-fatal):", err.message)
    );
    ...
    initiateWorkflow({
      ...
      console.error("[SOS] Police workflow error (in-memory path, non-fatal):", err.message)
    );
